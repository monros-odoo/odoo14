...
class SectionReportWizard(models.TransientModel):
    _name = 'sh.so.pos.section.report.wizard'

    #comman Fields
    date = fields.Date(string = "Date",default = date.today(),required=True)
    total_weeks = fields.Integer(string = "Total Weeks",required = True,default = 9)
    included_sale_order = fields.Boolean(string = "Included Sales Orders ?")
    included_pos_orders = fields.Boolean(string = "Included POS Orders ?")
    ...
    ],string = "Total Amount",default = 'untax_amount')
    ...
    def print_section_report(self):
        if self.total_weeks < 1:
            raise UserError("Please select atleast 1 week to print the report!!")

        if not self.included_sale_order and not self.included_pos_orders:
            raise UserError("Please Tick included sale or pos order to print the report")

        workbook = xlwt.Workbook()

        normal_record = xlwt.easyxf('font:height 210;align: vert center')
        
        header_record = xlwt.easyxf('font:height 300;align: horiz left;align: vert center;font:bold True')
        ...
        currency_style.num_format_str = str(self.company_id.currency_id.symbol)+"#,##0.00;"
        ...
        currency_style_sided.num_format_str = str(self.company_id.currency_id.symbol)+"#,##0.00;"
        ...
        first_day = (today - timedelta(days=today.weekday())) - timedelta(days=7 * (self.total_weeks - 1))
    
        self.env.cr.execute("""select name,from_time,to_time from sh_sale_pos_sector order by sequence;""")
        ...
        for week_count in range(int(range_value)):
            ...
            for sector in res:
                ...
                from_time = '{0:02.0f}:{1:02.0f}:00'.format(*divmod((sector[1]) * 60, 60))
                to_time = '{0:02.0f}:{1:02.0f}:59'.format(*divmod((sector[2]) * 60, 60))
                ...
                for i in range(display_week_no):
                    ...
                    day_start_day = user_tz.localize(fields.Datetime.from_string(day_start))
                    date_start = day_start_day.astimezone(pytz.timezone('UTC'))
                    day_start = fields.Datetime.to_string(date_start)
                    
                    user_tz = pytz.timezone(self.env.context.get('tz') or self.env.user.tz or 'UTC')
                    day_end_day = user_tz.localize(fields.Datetime.from_string(day_end))
                    date_end = day_end_day.astimezone(pytz.timezone('UTC'))
                    day_end = fields.Datetime.to_string(date_end)
                    
                    # if included pos order tick => find day_wise_total_pos
                    if self.included_pos_orders:
                        #users 
                        user_list = []
                        if self.user_ids:
                        ...
                        # if only one user then remove comma in tuple
                        if len(user_list) == 1:
                            listt = [(tuple(user_list))]
                            user_list = ', '.join('({})'.format(t[0]) for t in listt)
                        ...
                        if len(pos_config_list) == 1:
                            ...
                            pos_config_list = ', '.join('({})'.format(t[0]) for t in listt)
                        ...
                        day_wise_total_pos = self.env.cr.fetchall()
                        # print("\n\n\n\n\n day wise total",day_wise_total_pos)

                        if day_wise_total_pos[0][0]:
                            if i<7:
                                total += day_wise_total_pos[0][0]
                            else:
                                total1 += day_wise_total_pos[0][0]
                            day_total += day_wise_total_pos[0][0]

                    # if included so order tick => find day_wise_total_so
                    if self.included_sale_order:
                        ...
                        if self.sale_team_ids:
                            ...
                            crm_teams = self.env['crm.team'].search([])
                            team_list = crm_teams.ids

                        if len(team_list) == 1:
                            listt = [(tuple(team_list))]
                            team_list = ', '.join('({})'.format(t[0]) for t in listt)
                        ...
                        if self.total_amount == 'total_amount':
                            ...
                            self.env.cr.execute(""" select ROUND(sum(o.amount_total),2) from sale_order as o 
                                                    where o.state = 'sale' and o.invoice_status = '%s' and o.team_id in %s and date_order> '%s' 
                                                    and date_order<= '%s'; """ %(self.invoice_status,team_list,day_start,day_end))

                        else:

                            self.env.cr.execute(""" select ROUND(sum(o.amount_untaxed),2) from sale_order as o 
                                                    where  o.state = 'sale' and o.invoice_status = '%s' and o.team_id in %s and date_order> '%s' 
                                                    and date_order<= '%s'; """ %(self.invoice_status,team_list,day_start,day_end))

                        day_wise_total_so = self.env.cr.fetchall()

                        if day_wise_total_so[0][0]:
                            if i<7:
                                total += day_wise_total_so[0][0]
                            else:
                                total1 += day_wise_total_so[0][0]

                            day_total += day_wise_total_so[0][0]
            ...
            if week_count != (range_value -1) or  (self.total_weeks % 2) == 0 : # skip print in case of current week there is only 1 week
                    ...
                    if key == 'init':
                        ...
                        if x == 0:
                            ...
                            if col == 0:
                                ...
                                worksheet.col(col).width = 3000
                                worksheet.write(row, col,x, currency_style_sided)
                    col += 1
        ...
        worksheet.write_merge(0, 1, 0, 7, 'Sector Wise Sales Report ('+str(report_start_date)+' to '+str(report_end_date)+')', header_record)
        # worksheet.write_merge(0, 1, 9, 10, 'Status : '+ dict(self._fields['state'].selection).get(self.state), header_record)

        # listToStr = ''
        # if self.user_ids:
        #     listToStr = ', '.join([str(elem) for elem in self.user_ids.mapped('name')])

        #     worksheet.write_merge(0, 1, 11, 14, 'Users : '+ listToStr, header_record)
...
#             col = 19
            ...
            worksheet.write_merge(row, row, col, col+1, str(key)+' Total Sale', right_bold)
