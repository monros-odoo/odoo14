...
class BarcodeLabels(models.TransientModel):
                ...
                for line in purchase.order_line:
                    if line.product_id and line.product_id.type != 'service':
                        product_get_ids += [(0, 0, {
                            ...
                            'qty': int(line.product_qty) or 1.0
                        })]
                    ...
                    if line.product_id and line.product_id.type != 'service':
                        for i in line.move_line_nosuggest_ids:
                            if i.product_id.id == line.product_id.id :
                                product_get_ids += [(0, 0, {
                                    'product_id': line.product_id.id,
                                    'qty': int(i.qty_done) or 1.0,
                                    'lot_id': i.lot_id.id
                                })]

        elif self._context.get('active_model') == 'stock.inventory':
            record_ids = self._context.get('active_ids', []) or []
            picking_recs = self.env['stock.inventory'].browse(record_ids)
            product_get_ids = []
            for inv in picking_recs:
                for n in inv.line_ids:
                    print(n)
                    print(n.product_id)
                    if n.product_id and n.product_id.type != 'service':
                        product_get_ids += [(0, 0, {
                            'product_id': n.product_id.id,
                            'qty': int(n.product_qty) or 1 ,
                            'lot_id': n.prod_lot_id.id
                        })]
        elif self._context.get('active_model') == 'stock.move.line':
            record_ids = self._context.get('active_ids', []) or []
            stock_move_recs = self.env['stock.move.line'].browse(record_ids)
            product_get_ids = []
            for move in stock_move_recs:
                if move.product_id and move.product_id.type != 'service':
                    for inv in move.move_id.inventory_id.line_ids:
                        if inv.product_id.id == move.product_id.id:
                            product_get_ids += [(0, 0, {
                                'product_id': move.product_id.id,
                                'qty': int(inv.product_qty) or 1.0,
        ...
        datas = {
            ...
            'form': {
                ...
                'product_ids': [{
                    ...
                    'expir_date': line.lot_id and line.lot_id.expiration_date and str(
                        line.lot_id.expiration_date.month) + "/" + str(line.lot_id.expiration_date.year),
                    'lot_number': line.lot_id and line.lot_id.name or False,
                    'product_barcode': line.product_id.barcode or False,
                    'qty': line.qty,
    ...
    def print_report_zpl_lable(self):
        if not self.env.user.has_group('dynamic_barcode_labels.group_barcode_labels'):
            raise Warning(_("You have not enough rights to access this "
                            "document.\n Please contact administrator to access "
                            "this document."))
        ...
        datas = {
            ...
            'form': {
                ...
                'product_ids': [{
                    'product_id': line.product_id.id,
                    'lot_id': line.lot_id and line.lot_id.id or False,
                    'expir_date':line.lot_id.expiration_date or False,
                    'product_barcode':line.product_id.barcode or False,
                    'lot_number': line.lot_id and line.lot_id.name or False,
                    'qty': line.qty,
                    'company': self.env.company.name,
                    # 'symbol': line.product_id.company_id.currency_id.name,
                } for line in self.product_get_ids]
            }
        ...
        print(datas)
        return self.env.ref('dynamic_barcode_labels.barcodelabelzpl').report_action([], data=datas)
